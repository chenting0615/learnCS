     ...
     public Token evaluate () {
          ...
          while (true) {
               ...
               if (!expressOK(stack[top].type, tokens[nexttoken].type)) {
                    U.error ("Illegal expression: stack top="+
...
//             if (temptok.type == Token.ERROR) ???
     ...
     private Token compute (Token token1, Token operator, Token token2) {
          ...
          if (token1.type == Token.ID) {
               ...
               if (t1.type == Token.ERROR)
                    return t1;
          }
          if (token2.type == Token.ID) {
               t2 = resolveVariable(token2);
               if (t2.type == Token.ERROR)
                    return t2;
          }
          if (token1.type == Token.ARRAYREF) {
     ...
     private boolean setArrayValue (Token token1, Token token2) {
...
//                  return new Token(Token.INT, 0);
                    return new Token(Token.ERROR, "Variable "+varname+" is not defined.");
     ...
     public static boolean isLogOp (String tokenValue) {
