...
public class ProgElement {
     ...
     public Token evaluate (NameDB ndb) {
          return evaluate (ndb, false);
     }

     public Token evaluate (NameDB ndb, boolean definesOnly) {
          Palgo pp = ndb.parent;
          if (!definesOnly && type.equals("statement")) {
          ...
          if (type.equals("sequence")) {
               Token tok = null;
               for (int i=0; i<body.size(); i++) { 
                    tok = ((ProgElement)(body.elementAt(i))).evaluate(ndb, definesOnly);
                    if (tok.returnType.length() > 0)
                         return tok;
               }
               return tok;
          }
          if (!definesOnly && type.equals("while")) {
          ...
          if (!definesOnly && type.equals("repeat")) {
               Token tok = new Token(Token.INT, 0);  //the default
